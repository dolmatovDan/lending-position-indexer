# Delta: Эксплуатация — startup, docker-compose, конфиг, логи, тесты, README

**Change ID:** `add-lending-positions-indexer`
**Affects:** `cmd/positions` (`main.go`, `config.go`, `setup_*.go`), `Dockerfile`, `docker-compose.yml`, `.env.example`, `README.md`, тесты
