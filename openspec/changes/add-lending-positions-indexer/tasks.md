...
- [ ] 1.1 Инициализировать git-репозиторий: `git init`, `.gitignore` (Go/bin/`.env`),
...
- [ ] 1.2 Инициализировать модуль: `go.mod` (Go 1.22+), структура по ARCH: `cmd/positions`,
      `internal/{api/http,clients,managers,repositories,models,protocols,bindings}`, `abi/`,
      `migrations/postgres/`.
- [ ] 1.3 `cmd/positions/config.go`: `Config` struct с env-тегами (`RPC_WS_URL`,
      `RPC_HTTP_URL`, `WALLETS`, `CONFIRMATIONS`, `EULER_SUBACCOUNTS_DEPTH`, `DATABASE_URL`,
      `LOG_LEVEL`, `HTTP_PORT`); загрузка и валидация.
- [ ] 1.4 `internal/models`: `Position` (leg-модель: строка на актив — `token`, `side`,
...
- [ ] 1.5 `internal/bindings`: ABI-файлы в `abi/` (Aave Pool, AaveProtocolDataProvider,
      AaveOracle, Euler EVC, Euler EVault, EulerOracle, ERC20) + `go:generate` abigen.
- [ ] 1.6 `cmd/positions/setup_core.go`: инициализация `log/slog` (JSON, уровень из конфига).

**Quality Gate:**
- [ ] `go vet ./...` / `go build ./...` проходят
- [ ] Unit-тесты конфига зелёные

---

## Phase 2: Clients (ethnode, протоколы, pricing)

- [ ] 2.1 `internal/clients/ethnode`: обёртка ethclient — `SubscribeNewHead` (ws) + fallback-
      поллинг (http); caller с фиксацией block number; block number/timestamp. Interface + stub.
- [ ] 2.2 `internal/protocols`: общий интерфейс `Reader` (`Name`, `Positions(ctx, wallets, block) []models.Position`)
      — возвращает leg-строки уже с разрешёнными ценами.
- [ ] 2.3 `internal/pricing`: общие helpers — нормализация decimals/масштабирование, чтение
      Chainlink-агрегатора; используются адаптерами (отдельного pricing-клиента нет).
- [ ] 2.4 `internal/clients/aave`: адаптер Aave v3 — `getReservesList`, `getUserReserveData`,
      `getUserAccountData` (HF аккаунта), цены через AaveOracle; формирует leg-строки
      (side collateral/debt). Interface + stub.
- [ ] 2.5 `internal/clients/euler`: адаптер Euler v2 — дискавери через `EVC.getControllers`/
...
- [ ] 2.6 Unit-тесты: математика HF (Aave и Euler), конвертация активов, масштабирование цен,
      формирование leg-строк — на эталонных значениях, со stub'ами клиентов.

**Quality Gate:**
- [ ] Оба адаптера покрыты тестами на расчёты
- [ ] Нет обращений к внешним ценовым API (проверка по коду/зависимостям)
